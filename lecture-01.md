...
**Клієнт** є ініціатором взаємодії. Це може бути веббраузер, мобільний додаток, desktop застосунок або навіть інший сервер, що виступає у ролі клієнта. Основні обов'язки клієнта включають:
...
#### Stateless vs Stateful підходи

**Stateless підхід** означає, що сервер не зберігає інформацію про попередні запити від клієнта. Кожен запит містить всю необхідну інформацію для його обробки. Переваги цього підходу:
...
**Stateful підхід** передбачає, що сервер зберігає інформацію про сесію клієнта між запитами. Це може бути зручно для деяких застосунків, але створює складнощі:

- Прив'язка клієнта до конкретного серверу (session affinity).
...
У сучасній веброзробці переважає stateless підхід, особливо для RESTful API. Стан управляється на стороні клієнта або в розподілених системах кешування, а не на серверах застосунків.
...
**Request Line (стартова рядок)** містить три елементи:
...
**Headers (заголовки)** надають додаткову інформацію про запит:
...
**Body (тіло запиту)** містить дані, які надсилаються серверу. Воно присутнє у запитах POST, PUT, PATCH:
...
**GET** використовується для отримання ресурсу. Запити GET повинні бути безпечними (не змінювати стан сервера) та ідемпотентними. Параметри передаються у query string:
...
**CORS заголовки** управляють cross-origin requests:
...
**Client-Server.** Розділення інтерфейсу користувача від зберігання даних покращує портативність інтерфейсу та масштабованість серверних компонентів.
...
#### Error handling та стандартизація відповідей
...
Сучасні бібліотеки (Pydantic у Python, Jackson у Java, System.Text.Json у .NET) автоматично виконують serialization/deserialization з валідацією.

---

### 2.3. Dependency Injection pattern

Dependency Injection є фундаментальним патерном для створення loosely coupled та testable коду.
...
Data Access Layer ізолює решту застосунку від деталей бази даних. Можна змінити БД без впливу на бізнес-логіку.
...
**Reusability** - бізнес-логіка може використовуватися різними presentation layers (REST API, GraphQL, CLI, message queues).
...
- Dependency Injection робить код testable та maintainable.
- Layered Architecture організує код за відповідальностями.
...
6. Що таке Dependency Injection? Які переваги він надає для тестування?

7. Які шари включає Layered Architecture? Яка відповідальність кожного шару?
